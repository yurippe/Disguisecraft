    		...
    		public void run() {
    			...
    			if (observer.hasPermission("disguisecraft.seer")) {
    				...
    				if (pluginSettings.noTabHide && protocolHook == ProtocolHook.ProtocolLib) {
    					...
    					//toSend.add(new PacketPlayOutPlayerInfo(player.getName(), true, ((CraftPlayer) player).getHandle().ping)); //original code
    					EntityPlayer ep = ((CraftPlayer) player).getHandle(); //new code
						toSend.add(PacketPlayOutPlayerInfo.addPlayer(ep)); //new code
    				}
    			} else {
    	...
    	for (UUID disguisedUID : disguiseDB.keySet()) {
			...
			if (disguised != null && disguised != observer) {
				if (disguised.getWorld() == observer.getWorld()) {
					...
					if (pluginSettings.noTabHide && protocolHook == ProtocolHook.ProtocolLib) {
						/**
						((CraftPlayer) observer).getHandle().playerConnection.sendPacket(
								new PacketPlayOutPlayerInfo(getServer().getPlayer(disguisedUID).getName(), true, 
								((CraftPlayer) disguised).getHandle().ping));
								//original code
						*/
    					EntityPlayer ep = ((CraftPlayer) disguised).getHandle(); //new code
    					((CraftPlayer) observer).getHandle().playerConnection.sendPacket(PacketPlayOutPlayerInfo.addPlayer(ep)); //new code
						}
				}
			}
		}
