...
public class DCPacketGenerator {
	...
	public PacketPlayOutEntityLook getEntityLookPacket(Location loc) {
		byte[] yp = getYawPitch(loc);
		return new PacketPlayOutEntityLook(d.entityID, yp[0], yp[1], true);
	}
	...
	public PacketPlayOutEntityTeleport getEntityTeleportPacket(Location loc) {
		...
		return new PacketPlayOutEntityTeleport(d.entityID, x, y, z, yp[0], yp[1], true, true);
	}
	...
	public PacketPlayOutPlayerInfo getPlayerInfoPacket(Player player, boolean show) {
		if (d.type.isPlayer()) {
			...
			if (show) {
				//ping = ((CraftPlayer) player).getHandle().ping; //original code
				return PacketPlayOutPlayerInfo.addPlayer(ep);
			} else {
				//ping = 9999; //original code
				return PacketPlayOutPlayerInfo.removePlayer(ep); //new code
			}
			//return new PacketPlayOutPlayerInfo(d.data.getFirst(), show, ping); //original code
